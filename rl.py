...
OBSERVEPERIOD = (
    750  # Period actually start real Training against Experieced Replay Batches
)
BATCH_SIZE = 128
...
class Agent:
    ...
    def CaptureSample(self, sample):  # in (s, a, r, s_) format
        ...
        if self.steps > OBSERVEPERIOD:
            self.epsilon = MIN_EPSILON + (MAX_EPSILON - MIN_EPSILON) * math.exp(
                -LAMBDA * (self.steps - OBSERVEPERIOD)
            )
    ...
    def Process(self):
        ...
        states_ = numpy.array(
            [
                (no_state if batchitem[3] is None else batchitem[3])
                for batchitem in batch
            ]
        )

        predictedQ = self.brain.predict(
            states
        )  # Predict from keras Brain the current state Q Value
        predictedNextQ = self.brain.predict(
            states_
        )  # Predict from keras Brain the next state Q Value
        ...
        for i in range(batchLen):
            ...
            if nextstate is None:
                ...
                targetQ[a] = reward + GAMMA * numpy.amax(
                    predictedNextQ[i]
                )  # The core Q[S,A] Update recursive formula
    ...
    def saveModel(self, name):
        self.brain.model.save(f"models/{name}.keras")
